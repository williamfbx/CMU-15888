...
# Global configuration: Pretrained base model for best response training
PRETRAINED_BASE_MODEL = 'handover-sim/GA-DDPG/output/27_11_2025_10:25:09'
...
class PSROTrainer:
    ...
    def _train_best_response_agent(self, iteration, adversary_meta_strategy):
        ...
        # Use the global pretrained base model
        global PRETRAINED_BASE_MODEL
        if PRETRAINED_BASE_MODEL is None:
            raise ValueError("PRETRAINED_BASE_MODEL is not set.")
        
        pretrained_model = os.path.basename(PRETRAINED_BASE_MODEL)
        self._log(f"Using pretrained base model: {pretrained_model}")
        self._log(f"  Full path: {os.path.abspath(PRETRAINED_BASE_MODEL)}")
        ...
        self._log(f"Running training command with pretrained base model...")
        result = subprocess.run(cmd, capture_output=False)
        if result.returncode != 0:
...
def main():
    ...
    global PRETRAINED_BASE_MODEL
    print(f"Using pretrained base model: {PRETRAINED_BASE_MODEL}")
